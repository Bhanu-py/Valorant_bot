...
async def parse_stats(user_id, headers, num_matches=3):
    try:
        async with aiohttp.ClientSession() as session:
            headers["X-Riot-ClientVersion"] = "release-03.06-shipping-8-610061"
            headers[
                "X-Riot-ClientPlatform"
            ] = "ew0KCSJwbGF0Zm9ybVR5cGUiOiAiUEMiLA0KCSJwbGF0Zm9ybU9TIjogIldpbmRvd3MiLA0KCSJwbGF0Zm9ybU9TVmVyc2lvbiI6ICIxMC4wLjE5MDQyLjEuMjU2LjY0Yml0IiwNCgkicGxhdGZvcm1DaGlwc2V0IjogIlVua25vd24iDQp9"
            async with session.get(
                f"https://pd.AP.a.pvp.net/mmr/v1/players/{user_id}/competitiveupdates?startIndex=0&endIndex=20",
                headers=headers,
            ) as r:
...
async def get_rank(user_id, headers):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://pd.AP.a.pvp.net/mmr/v1/players/{user_id}", headers=headers
            ) as r:
                data = json.loads(await r.text())
                pp.pprint(headers)
            pp.pprint(data)
            ...
            rank_tier = latest['TierAfterUpdate']
            ranked_rating = latest['RankedRatingAfterUpdate']
...
async def get_user(user_id, headers):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.put(
            f"https://pd.AP.a.pvp.net/name-service/v2/players",
            headers=headers,
...
async def match_data(user_id, headers, match_id):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://pd.AP.a.pvp.net/match-details/v1/matches/{match_id}",
                headers=headers,
            ) as r:
                data = json.loads(await r.text())
            # print(data)
...
if __name__ == "__main__":
     user_id, headers = asyncio.get_event_loop().run_until_complete(run('bhn4477', 'Bhanu3003'))
    # # print(asyncio.get_event_loop().run_until_complete(parse_stats(user_id, headers)))
    # print(asyncio.get_event_loop().run_until_complete(match_data(user_id, headers, 'd7be293e-e3fd-41e0-9ff0-254c14417ec5')))
